...
# def lidar2world(lidar_hit,joint_angles,pose=None, Particles=None):
    ...
    roll_gb = body_angles[0]        # roll angle wrt global frame
    pitch_gb = body_angles[1]       # pitch angle wrt global frame
    yaw_gb = body_angles[2]         # yaw angle wrt global frame   

    #homogeneous transformation from body to lidar
    body_2_lidar_rot = rot_z_axis(neck_angle) @ rot_y_axis(head_angle)
    body_2_lidar_trans = np.array([0,0,0.15])
    body_2_lidar_homo = homo_transform(body_2_lidar_rot,body_2_lidar_trans)
    # homogeneous transformation from world to body, then world to lidar
    if Particles is None:   # for mapping
        x_gb = pose[0]      # x position of body wrt world/global frame
        y_gb = pose[1]      # y position of body wrt world/global frame
        z_gb = 0.93
        # don't use the IMU roll, pitch, yaw data. Use pose data only?
        world_2_body_rot = euler_matrix(roll_gb,pitch_gb,yaw_gb)[:-1,:-1]
        world_2_body_trans = np.array([x_gb,y_gb,z_gb])
        world_2_body_homo = homo_transform(world_2_body_rot,world_2_body_trans)
        world_2_lidar_homo = world_2_body_homo @ body_2_lidar_homo
        lidar_hit_augment = np.vstack((lidar_hit,np.ones((1,lidar_hit.shape[1]))))
        # print('world_2_lidar_homo',world_2_lidar_homo)
        world_hit = world_2_lidar_homo @ lidar_hit_augment
        #  body wrt world
        ...
        lidar_hit_augment = np.vstack((lidar_hit, np.ones((1, lidar_hit.shape[1]))))
        for i in range(nums):
            world_2_body_trans = np.array([poses[0,i],poses[1,i],0.93])
            # don't use the IMU roll, pitch, yaw data. Use pose data only
            # world_2_body_rot = rot_z_axis(poses[-1,i])
            world_2_body_rot = euler_matrix(roll_gb,pitch_gb,yaw_gb)[:-1,:-1]
            world_2_body_homo = homo_transform(world_2_body_rot,world_2_body_trans)
            world_2_lidar_homo = world_2_body_homo @ body_2_lidar_homo
            world_hit = world_2_lidar_homo @ lidar_hit_augment
            world_hit = world_hit[:3,:]
            not_floor = world_hit[2]>0.1
            particles_hit.append(world_hit[:, not_floor])
        return np.transpose(np.asarray(particles_hit), (1,2,0))
def world2map(xy,Map):
    # return the occupancy index (int) from world_hit(meter,float)
    # transform origin from center to upper left
    pixels = np.zeros(xy.shape, dtype=int)      #shape: 2*n
    pixels[0] = ((xy[0] + (Map['xmax']-Map['xmin'])/2)/Map['res']).astype(int)
    pixels[1] = ((-xy[1] + (Map['ymax']-Map['ymin'])/2)/Map['res']).astype(int)
    # check boundary and keep pixels within
    center_x = (Map['xmax']-Map['xmin'])/Map['res']/2
    center_y = (Map['ymax']-Map['ymin'])/Map['res']/2
    in_bound = np.logical_and(np.abs(pixels[0]-center_x) < center_x, np.abs(pixels[1]-center_y) < center_y)
