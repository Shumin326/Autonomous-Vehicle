...
def genMap(slam_inc, end_t=None):
    ...
    # print(f'best_p_indices.shape={best_p_indices.shape}')
    t0             = slam_inc.t0
    if end_t is None:
        end_t          = slam_inc.num_data_ - 1
    MAP_2_display = np.zeros((MAP['map'].shape[0],MAP['map'].shape[1],3),dtype=np.uint8)
    occ_mask = MAP['map']>MAP['occ_thres']
    free_mask = MAP['map']<MAP['free_thres']
    und_mask = np.logical_not(np.logical_or(occ_mask, free_mask))
    MAP_2_display[occ_mask] = [0,0,0]            # black for occ
    MAP_2_display[free_mask] = [255,255,255]     # white for free
    MAP_2_display[und_mask] = [128,128,128]      # gray for und
    # MAP_2_display[occ[1], occ[0]] = [0, 255, 0]  # green for lidar
    # cv2_imshow(MAP_2_display)
    # cv2.waitKey(10)
    ...
    # print(f'best_p_indices={best_p_indices}')
